...
//! スクリプトエンジンが動作する様にします。
//!
bool GameScriptEngine::Initialize( const TCHAR* path )
{
	...
	if ( !scripts_data->LoadJson( path,"scripts" ) )
	{
		return false;
	}
...
//! @return ESC キーが押されたか
//!
bool GameScriptEngine::IsExit() const
{
...
//! @return 処理の成否
//!
bool GameScriptEngine::InitializeCursor()
{
	auto handle = 0;
...
//! @return 処理の成否
//!
bool GameScriptEngine::InitializeClickWait()
{
	auto handle = 0;
...
//! @return 処理の成否
//!
bool GameScriptEngine::InitializeStrings()
{
...
//!
void GameScriptEngine::Update( Game& _game )
{
...
//!
void GameScriptEngine::UpdateMessage()
{
	is_click_wait_visible = false;
...
//!
void GameScriptEngine::OnCommandClick()
{
...
//! @return 処理の成否
//!
bool GameScriptEngine::OnCommandImage( unsigned int line,const std::vector<std::string>& scripts )
{
...
//! @return 処理の成否
//!
bool GameScriptEngine::OnCommandMessage( unsigned int line,const std::vector<std::string>& scripts )
{
...
//! @brief クリック待ち処理
//!
void GameScriptEngine::ClickWait()
{
...
//! @brief 選択待ち処理
//!
void GameScriptEngine::ChoiceWait()
{
...
//!
bool GameScriptEngine::CalculateMessageArea( const std::string& message,Rect& area,int& right_goal )
{
	if ( message.empty() )
	{
		return false;
...
//! @return 処理の成否
//!
bool GameScriptEngine::GetLineNumber( const std::string& str,unsigned int& line ) const
{
...
//! 画像ロード関数で得られる描画用の値です。
//!
bool GameScriptEngine::GetImageHandle( const std::string& str,int& handle ) const
{
...
//! @details 毎フレーム呼び出す必要があります。
//!
void GameScriptEngine::Draw( Game& _game ) const
{
	RenderImage();
	RenderMessageWindow();
	RenderMessage();
...
//! @brief マウスカーソル画像の描画
//!
void GameScriptEngine::RenderCursor() const
{
...
//! @brief 'd' コマンドによる画像描画
//!
void GameScriptEngine::RenderImage() const
{
...
//!
void GameScriptEngine::RenderMessageWindow() const
{
	SetDrawBlendMode( DX_BLENDMODE_ALPHA,64 );

	DrawBox( message_window_left,MSG_WINDOW_TOP,message_window_right,MSG_WINDOW_BOTTOM,message_window_color,TRUE );
...
//!
void GameScriptEngine::RenderMessage() const
{
	for ( auto&& message : message_list )
	{
		const auto area = message->GetArea();
...
//!
void GameScriptEngine::RenderChoice() const
{
		// 先に選択エリアを描画してしまう
	for ( auto&& choice : choice_list )
	{
	...
	for ( auto&& choice : choice_list )
	{
		...
		DrawString( area.left,area.top,choice->GetMessage().c_str(),message_string_color );
	}
}
