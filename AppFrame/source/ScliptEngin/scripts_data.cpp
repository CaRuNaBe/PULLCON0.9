...
void ScriptsData::LoadJson ( std::string storyname )
{
	 // UTF-8 BOM無し Json file
	std::ifstream ifs ( SCRIPTS_JSON_PATH );
	picojson::value json_value;

	ifs >> json_value;

	auto root = json_value.get<picojson::array> ();
	auto _object = root [ 0 ].get<picojson::object> ();
	const auto json_array = _object [ storyname ].get<picojson::array> ();
